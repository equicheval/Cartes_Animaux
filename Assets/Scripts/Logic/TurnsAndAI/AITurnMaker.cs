...
public class AITurnMaker: TurnMaker {
    ...
    public override void OnTurnStart()
    {
        ...
        new ShowMessageCommand("Tour adverse", 2.0f).AddToQueue();
        p.DrawACard();
    ...
    bool UseHeroPower()
    {
        if (p.RessourceSunThisTurn >= 2 && !p.usedHeroPowerThisTurn)
        {
            // use HP
            p.UseHeroPower();
            InsertDelay(1.5f);
